...
const FeatureCard = ({ icon, title, description }) => (
  <motion.div
    ...
    viewport={{ once: true }}
    className="bg-[#2E2E2E] p-5 rounded-xl shadow-lg flex flex-col items-start text-left sm:items-center sm:text-center hover:shadow-xl transition-shadow"
  >
    <div className="p-3 bg-[#00FF84]/10 rounded-full mb-3">{icon}</div>
...
const About = () => {
  const benefits = [
    ...
    {
      icon: <DollarSign className="w-6 h-6 text-[#00FF84]" />,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <div className="w-full lg:w-1/2">
            ...
            <p className="text-base text-[#BBBBBB] leading-relaxed mb-4">
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              {["Integrity", "Honesty", "Trust", "Quality"].map((value, idx) => (
                <span key={idx} className="px-3 py-1 text-sm rounded-full bg-[#00FF84]/10 text-[#00FF84] font-medium">
                  {value}
                </span>
              ))}
            </div>
        ...
        <motion.div
          ...
          className="mt-20"
        >
          <h3 className="text-2xl md:text-3xl font-bold text-[#F5F5F5] mb-6 text-center">
            Why Choose Us?
          </h3>
